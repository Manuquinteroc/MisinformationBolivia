...
for (x in aux) {
  ...
  if (count == 1) {
    assign(nam1, lm(fmla1, data = final_data_attrition))  
  } else {
    assign(nam1, lm(fmla1, data = aux_post_data))
  }
...
table1 <- stargazer(lm_list_ols,
                    ...
                    add.lines = list(c("Outcome mean", round(means, 3)),
                                     c("Control mean", round(means_control, 3))),
...
The variable ", interaction_name, " is an indicator for responding the WhatsApp final_data_attrition. ", additional_note,
...
subcaption <- paste0("[-0.5ex] \\multicolumn{", size, "}{l}{\\textbf{A.2 Sample Course Delivery WhatsApp}} \\\\[-1.8ex] \\\\ \\cline{1-2} \\\\[-0.5ex]")
