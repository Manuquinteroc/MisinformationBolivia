...
for (x in aux) {
  ...
  # fmla2 <- as.formula(paste0(x, "~ ", paste(treatments_separate, collapse = " + "), " + ",
  #                            paste(paste0(treatments_separate, ":", interacted), collapse = " + "), " + ",
  #                            paste(control_list[[count]], collapse = " + ")))
  # 
  nam1 <- paste("lm_", count, "_ols", sep = "")
  assign(nam1, lm(fmla1, data = survey))
  #assign(nam1, lm_robust(fmla2, se_type = "HC0", fixed_effects = blockid1,  data = survey))
  
  
  lm_list_ols[[count]] <- get(nam1, envir = globalenv())
  #lm_list_ols2[[count]] <- get(nam1, envir = globalenv())
