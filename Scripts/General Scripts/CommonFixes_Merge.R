...
names(survey_wa)[which(names(survey_wa) == "freq_veri_compar")] <- "freq_veri_compar" # baseline is with the same name
...
survey_wa <- survey_wa %>% dplyr::select(all_of(c(wa_vars, wa_indexes))) %>% dplyr::rename(blockid1 = blockid4)
...
survey_alf <- survey_alf %>% dplyr::select(all_of(c(alf_vars, alf_indexes))) 
