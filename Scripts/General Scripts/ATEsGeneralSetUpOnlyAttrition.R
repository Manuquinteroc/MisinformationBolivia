...
if (length(aux_data) == 1) {
  means <- round(mean(aux_data[which(survey$control == 1),], na.rm = T), 3)
  sds <- round(sd(as.matrix(aux_data[which(survey$control == 1),]), na.rm = T), 3)
  
} else {
  means <- round(colMeans(aux_data[which(survey$control == 1),], na.rm = T), 3)
  sds <- round(colSds(as.matrix(aux_data[which(survey$control == 1),]), na.rm = T), 3)
}
...
The variable ", interaction_name, " is an indicator for responding the WhatsApp survey. ", additional_note,
