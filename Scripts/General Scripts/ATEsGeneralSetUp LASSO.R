...
for (x in aux) {
  fmla1 <- as.formula(paste0(x, "~ ", paste(treatments_separate, collapse = " + "), " + ",
                             ...
                             paste(control_list[[count]], collapse = " + "), "+ factor(blockid1)"))
  
  nam1 <- paste("lm_", count, "_ols", sep = "")

  if(weight1 == TRUE) {
    assign(nam1, lm(fmla1, weights = weight1, data = survey))
    
  } else if(weight2 == TRUE) {
    assign(nam1, lm(fmla1, weights = weight2, data = survey))
...
All specifications in all panels include the corresponding outcome variables at baseline as a control.
Specifications in Panel A and B further include covariates in the outcome family as controls.
Specifications in Panel A and B further include covariates in the outcome family as controls.
Specifications in Panel C are from a 10-fold cross validation LASSO model with lambda chosen to be that of the minimum average cross-validated error. 
The treatment indicators, lagged dependent variable, and fixed effects are forced into the LASSO model and covariates are selected from a pool of ", number_lasso," baseline variables for each specification. 
...
# Append caption for sub-panel 2: WhatsApp
subcaption <- paste0("[-0.5ex] \\multicolumn{", size, "}{l}{\\textbf{C.2 Sample Course Delivery WhatsApp}} \\\\[-1.8ex] \\\\ \\cline{1-2} \\\\[-0.5ex]")
...
aux1_original <- sub("\\\\\\\\\\[-1.8ex] Outcome mean.*", "\\\\\\\\", table1)

aux2_original <- sub(".*\\\\\\\\\\[-1.8ex] Outcome mean", "\\\\\\\\ \\\\hline \\\\\\\\\\[-1.8ex] Outcome mean", table1)
