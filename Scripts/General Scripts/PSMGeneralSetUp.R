...
for (x in aux) {
  ...
  if(weight1 == TRUE) {
    assign(nam1, lm(fmla1, weights = pr_score*weight1, data = df_aux))
    
  } else if(weight2 == TRUE) {
    assign(nam1, lm(fmla1, weights = pr_score*weight2, data = df_aux))
    
  } else {
    assign(nam1, lm(fmla1, weights = pr_score, data = df_aux))
