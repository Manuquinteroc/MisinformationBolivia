...
c1 <- as.numeric(c(lm_ate[[1]]$coefficients[1:3], lm_ate[[3]]$coefficients[1:3], lm_ate[[5]]$coefficients[1:3], lm_ate[[6]]$coefficients[1:3]))
c2 <- as.numeric(c(lm_ate[[1]]$conf.low[1:3], lm_ate[[3]]$conf.low[1:3], lm_ate[[5]]$conf.low[1:3], lm_ate[[6]]$conf.low[1:3]))
c3 <- as.numeric(c(lm_ate[[1]]$conf.high[1:3], lm_ate[[3]]$conf.high[1:3], lm_ate[[5]]$conf.high[1:3], lm_ate[[6]]$conf.high[1:3]))
...
Figure3A <- ggplot(data_aux, aes(treatment, coefs), color = factor(treatment)) + 
  ...
  geom_errorbar(aes(ymin = left, ymax = right, color = factor(treatment)), width = 0.2,
                cex = 0.75) +
  facet_grid(. ~ variables, labeller = labeller(variables = label_wrap_gen(23))) +
...
Figure4A <- ggplot(data_aux, aes(treatment, coefs), color = factor(treatment)) + 
  ...
  geom_errorbar(aes(ymin = left, ymax = right, color = factor(treatment)), width = 0.2,
                cex = 0.75) +
  facet_grid(. ~ variables, labeller = labeller(variables = label_wrap_gen(30))) +
  scale_x_discrete(limits = positions) +
  theme_light() +
  th + 
  scale_color_manual(name = "Treatment",  labels = c("Course and verification", "Course", "Verification"),
...
# Figure 5A: consumption, sharing, and verifying behavior
c1 <- as.numeric(c(lm_ate[[10]]$coefficients[1:3], lm_ate[[11]]$coefficients[1:3], lm_ate[[12]]$coefficients[1:3]))
c2 <- as.numeric(c(lm_ate[[10]]$conf.low[1:3], lm_ate[[11]]$conf.low[1:3], lm_ate[[12]]$conf.low[1:3]))
c3 <- as.numeric(c(lm_ate[[10]]$conf.high[1:3], lm_ate[[11]]$conf.high[1:3], lm_ate[[12]]$conf.high[1:3]))
c4 <- rep(c("Course and \nverification", "Course", "Verification"), 3)
c5 <- c(rep("Decrease consumption behavior of social media sources index",3),
...
c1 <- rbind.data.frame(lincom(lm_ate[[10]], test, level = 0.90)[,1:3],
...
# Appendix of traditional sources ----------------------------------------------
c1 <- as.numeric(c(lm_ate[[2]]$coefficients[1:3], lm_ate[[4]]$coefficients[1:3], lm_ate[[9]]$coefficients[1:3]))
c2 <- as.numeric(c(lm_ate[[2]]$conf.low[1:3], lm_ate[[4]]$conf.low[1:3], lm_ate[[9]]$conf.low[1:3]))
c3 <- as.numeric(c(lm_ate[[2]]$conf.high[1:3], lm_ate[[4]]$conf.high[1:3], lm_ate[[9]]$conf.high[1:3]))
c4 <- rep(c("Course and \nverification", "Course", "Verification"), 3)
c5 <- c(rep("Decrease likelihood of false of traditional sources index",3),
        rep("Trust traditional sources index",3),
        rep("Increase consumption behavior of traditional sources index",3))
...
Figure6A <- ggplot(data_aux, aes(treatment, coefs), color = factor(treatment)) + 
  geom_point(aes(color = factor(treatment)), size = 4, fill = "#d3d3d3") + 
  geom_errorbar(aes(ymin = left, ymax = right, color = factor(treatment)), width = 0.2,
                cex = 0.75) +
  facet_grid(. ~ variables, labeller = labeller(variables = label_wrap_gen(30))) +
...
ggsave(Figure6A, path = 'Figures', filename = "Figure6A.pdf", device = cairo_pdf, 
       width = 12, height = 8, dpi = 300)

# Figure 6B: mechanisms
c1 <- rbind.data.frame(lincom(lm_ate[[2]], test, level = 0.90)[,1:3],
                       lincom(lm_ate[[4]], test, level = 0.90)[,1:3],
                       lincom(lm_ate[[9]], test, level = 0.90)[,1:3])
...
Figure6B <- ggplot(data_aux, aes(treatment, coefs), color = factor(treatment)) + 
  geom_point(aes(color = factor(treatment)), size = 4, fill = "#d3d3d3") + 
  geom_errorbar(aes(ymin = left, ymax = right, color = factor(treatment)), width = 0.2,
                cex = 0.75) +
  facet_grid(. ~ variables, labeller = labeller(variables = label_wrap_gen(30))) +
...
ggsave(Figure6B, path = 'Figures', filename = "Figure6B.pdf", device = cairo_pdf, 
       width = 12, height = 8, dpi = 300)
...
figureA3_a <- ggplot(data_aux, aes(x = treatment, y = coefs, colour = method)) +
  ...
  geom_errorbar(aes(ymin = left, ymax = right), width = 0.5,
                cex = 0.75, position =position_dodge) +
  facet_grid(. ~ variables,  labeller = labeller(variables = label_wrap_gen(40))) +
  theme_light() +
  th +
  scale_color_manual(name = "Method",  labels = c("OLS", "IPSW", "LASSO"),
                     values = colors) +
