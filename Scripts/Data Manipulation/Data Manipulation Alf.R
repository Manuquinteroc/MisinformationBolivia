# Data wrangling for the Online survey responses 

# ------------------------------------------------------------------------------
# read data frame
survey <- read_dta('Datasets/Alfabetizacion/final_data_alfabetizacion.dta', encoding = "UTF-8") 

# drop variables
survey <- survey %>% dplyr::select(-c(39,40,42,46:54)) # col 1,3,4, 11 are email, name, lastname, and celphone

# Attrition --------------------------------------------------------------------

# change format of dataframe from long to wide
baseline_attrition <- subset(survey, time == 1)
# notice that these 5 responses have all missing survey responses (we use gender to delete these 5 obs)
ind <- which(baseline_attrition$baseline == 1 & is.na(baseline_attrition$missing_baseline) & !is.na(baseline_attrition$gender))
baseline_attrition <- baseline_attrition[ind,]
...
write.csv(survey_attrition, 'Datasets/Alfabetizacion/DataForAttrition.csv', row.names = F)
...
names(survey)[(length(survey)- 2):length(survey)] <- c("only_news", "only_opinion", "opinion_both")
...
# Save data --------------------------------------------------------------------
write.csv(survey, 'Datasets/Alfabetizacion/survey_final.csv', row.names = F)
